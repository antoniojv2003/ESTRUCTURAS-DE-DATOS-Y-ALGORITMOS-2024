...
public class ListaEnlazada { //mediante esta clase implementaremos la lista enlazada
    ...
    //PARA EL TP3 LA REDEFINO PARA QUE SEA RECURSIVA SEGUN EL SIGUIENTE ENUNCIADO
    /*Rediseñar el método mostrar de su clase ListaEnlazada del TP n° 1 para que sea recursiva. Debe conservar la interfaz de la clase. 
    Calcule la complejidad en notación O grande. Implemente en Java.
    */
    public void mostrar()
    {
        if(this.esVacia())
        ...
        {
            ListaEnlazada Laux=ListaEnlazada.crearLista(); //lista enlazada auxiliar           
            Laux.primero=this.primero; //definimos así para que no se borren los elementos de la lista original
            System.out.println(Laux.primero.informacion);
            Laux.borrarPrimero();
            Laux.mostrar();
    ...
    //MÉTODO PARA EL PUNTO 3 DEL TP3: sumaEnteros
    /*Dada una lista enlazada de números enteros, escriba un algoritmo recursivo que calcule la suma de los enteros almacenados en los nodos cuyo valor sea superior a un valor umbral pasado como parámetro. 
      Calcule la complejidad en notación O grande. Implemente en Java. */
    
    public int sumaEnteros(int valorUmbral)
    {
        if(this.esVacia())
        {
            return 0;
        ...
        {
            ListaEnlazada Laux=ListaEnlazada.crearLista();
            Laux.primero=this.primero; 
            if(Laux.primero.informacion>valorUmbral)
            {
                int valor=Laux.primero.informacion; 
                Laux.borrarPrimero();
                return valor+Laux.sumaEnteros(valorUmbral); 
            }
            else
            {
                Laux.borrarPrimero();
                return Laux.sumaEnteros(valorUmbral);
            }
        }
